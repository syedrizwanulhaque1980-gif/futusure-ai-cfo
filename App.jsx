...
// ─────────────────────────────────────────────────────────────
const RAZORPAY_ME_LINK = "https://razorpay.me/@futusurebusinesssolutionspvtl";

function PlansScreen({ onPaid, onSkipDemo }) {
  const [selectedPlan, setSelectedPlan] = useState(null); // after user clicks Pay
  const [confirming, setConfirming] = useState(false);

  const openRazorpay = (planId) => {
    setSelectedPlan(planId);
    // Open your Razorpay.me link – user enters the amount shown on screen
    window.open(RAZORPAY_ME_LINK, "_blank", "noopener,noreferrer");
  };
  ...
  const confirmPayment = () => {
    ...
    const receipt = {
      ...
      planName: plan.name,
      amount: plan.price,
      paymentId: `rzp_me_${Date.now()}`,
      orderId: `fs_${selectedPlan}_${Date.now()}`,
      date: new Date().toISOString(),
      status: "paid",
      method: "Razorpay.me",
    };
    localStorage.setItem(PAID_KEY, JSON.stringify({ plan: selectedPlan, ...receipt }));
        ...
        {Object.values(PLANS).map((plan) => (
          ...
          >
            ...
            <div className="flex items-baseline justify-between">
              ...
              <div>
                <span className="text-xl font-semibold" style={{ color: ink }}>{plan.priceLabel}</span>
            ...
            <button
              className="mt-4 w-full py-2.5 rounded-md text-sm font-medium flex items-center justify-center gap-2"
              style={{
                background: plan.id === "monthly" ? gold : ink,
                color: "#FFFFFF",
              }}
              onClick={() => openRazorpay(plan.id)}
            >
              <CreditCard size={16} />
              Pay {plan.priceLabel} via Razorpay
            </button>
          </div>
        ))}
      </div>

      {/* After user opens Razorpay link */}
      {selectedPlan && (
        <div
          className="mt-5 rounded-lg p-4"
          style={{ background: "#FFF8EE", border: `1px solid ${gold}` }}
        >
          <p className="text-xs" style={{ color: ink }}>
            <strong>Next step:</strong> On the Razorpay page, enter exactly{" "}
            <strong>{PLANS[selectedPlan].priceLabel}</strong> and complete the payment.
          </p>
          <button
            className="mt-3 w-full py-2.5 rounded-md text-sm font-medium"
            style={{ background: moss, color: "#FFFFFF", opacity: confirming ? 0.7 : 1 }}
            disabled={confirming}
            onClick={confirmPayment}
          >
            {confirming ? "Unlocking…" : "I have completed the payment"}
          </button>
        </div>
      )}
